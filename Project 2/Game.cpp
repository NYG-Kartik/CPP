...
//    "Cell â€¦ is already chosen. Please re-enter: ", 
//    where â€¦ is the value of num
//
//    Re-enter num
//end
//return num
...
void Game::play() {
    ...
    int round = 1;
    int num;
    //cin >> num; //wrong, will be entered in input method
    int non_mines_checked = 0;
    int total_non_mines = size - area.get_num_mines();
    while(!end){
        //cout << "Round " << round << ":";
        cout << "Round " << round << ": "; //modified, space after :
        num = input();
        if(area.has_mine(num)){
            ...
            cout << area.to_string() << endl; //add, print the current status of field with newly checked non-mines

            if(non_mines_checked == total_non_mines){
